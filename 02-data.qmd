...
[Recording](https://uofglasgow.zoom.us/rec/share/o0RHg82nSsyPLSMpA5hxHdqiHnc86H7zmBvzDL-n6ICqq0NQ1wHf8FKF3zvkeGmj.H72yZQDGbVST5gqQ)
...
::: {.info}
I use `readr::read_csv()` so we can fix the column types, but you're free to use any way you're comfortable with to load and process the data.
:::
...
```{r}
# add to data-raw/self_res_att.R
ct <- cols(
  id = col_character(),
...
```{r, eval = FALSE}
# include in data-raw/self_res_att.R after prep code
usethis::use_data(self_res_att, overwrite = TRUE)
```
...
```{r}
# add this code to data-raw/self_res_att.R
vars <- c(
    id = "",
    sex = "",
    ethgroup = "",
