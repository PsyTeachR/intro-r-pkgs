...
```{r, eval = FALSE}
# replace contents of tests/testthat/apa_t_pair.R
...
Now we can edit the function `apa_t_pair()` (in the file `R/apa_t_pair.R`) until the test passes. Add a few lines of code at the start of the function to check if all of the values in `x` are the same as all of the values in `y`, and display a warning if they are.
...
Now that we have 5 tests for this function, we can run them all by clicking on **`Run Tests`** in the upper right corner of the `r glossary("panes", "source pane")`. If all goes well, you'll see the following in the Build tab:
...
```{r, eval = FALSE}
# add to tests/testthat/self_res_att.R
